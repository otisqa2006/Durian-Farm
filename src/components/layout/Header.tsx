...
export default function Header({ title, subtitle }: HeaderProps) {
  const { isBalanced, systemTotal } = useBalanceValidator();
    ...
    <header className="h-[var(--header-height)] flex items-center justify-between px-4 md:px-6 border-b border-border/50 bg-surface/80 backdrop-blur-lg sticky top-0 z-30">
      ...
      <div className="flex items-center gap-3">
        <div className={`flex items-center gap-2 px-3 py-1.5 rounded-full text-xs font-semibold ${
          ...
          <span className="hidden sm:inline">{isBalanced ? 'Cân đối' : 'Lệch!'}</span>
        </div>
        <div className="hidden md:flex flex-col items-end">
          <span className="text-[10px] text-muted uppercase tracking-wider">Tổng quỹ</span>
